...
@configclass
class DynamicObstacleTemplateSceneCfg(InteractiveSceneCfg):
    """Configuration for a scene for deploying dynamic obstacle"""
    ...
    contact_forces = ContactSensorCfg(prim_path="{ENV_REGEX_NS}/Robot/.*", history_length=3, track_air_time=True)# Robots 

    # Dynamic Obstacle
    dyn_obst: AssetBaseCfg = MISSING

    light = AssetBaseCfg(
        prim_path= "/World/skyLight",
        spawn=sim_utils.DomeLightCfg(color=(0.13, 0.13, 0.13), intensity=10000)
    )
...
@configclass
class SimpleDynObstSceneCfg(DynamicObstacleTemplateSceneCfg):
    
    dyn_obst = OBS_CFG.replace(
...
class CommandsCfg:
    ...
    Task using this base env will have the commands for:
     - simple obstacle to get to a goal position
     - robot to achieve goals along trajectory
    """

    # target position for the obstacle
    ...
    # from nav_task mdp.FixGoalCommand
    # fixed_goal_command = mdp.FixGoalCommandCfg(
    #     asset_name="robot",
    #     resampling_time_range=(60, 120),
        
    #     fix_goal_position=[0.0, 0.0, 0.0], # list[floats]=[x,y,theta]  
    # )
...
class ActionsCfg:
    ...
    Task using this base env will only have an action for the simple obstacle to move to a target via PD controller"""

    dyn_obst_pos_command = mdp.SimpleDynObstacleActionTermCfg(
        asset_name="dyn_obst",
        max_velocity=5,
        max_acceleration=5,
        max_rotvel=6,
...
class ObservationCfg:
    """Observation specification for the simple dynamic Obstacle MDP"""
    ...
    # for the obstacle control, with the current implementatin not needed really
    @configclass
    class ObstacleControlCfg(ObsGroup):
        """Observation for the simple obstacle"""

        position_commands = ObsTerm(func=mdp.generated_commands, params={"command_name": "dyn_obst_target_pos"})
    ...
    dyn_obstacle_control: ObstacleControlCfg = ObstacleControlCfg()
...
@configclass
class SimpleDynEnvCfg(ManagerBasedRLEnvCfg):
    """Configuration for the simple dynamic obstacle navigation environment"""

    # Scene settings
    scene: SimpleDynObstSceneCfg = SimpleDynObstSceneCfg(
        num_envs=4,
        env_spacing=5,
    )
...
@configclass
class SimpleDynEnvCfg_TRAIN(SimpleDynEnvCfg):
    def __post_init__(self):
        super().__post_init__()
...
@configclass
class SimpleDynEnvCfg_DEV(SimpleDynEnvCfg):
    def __post_init__(self):
        super().__post_init__()
