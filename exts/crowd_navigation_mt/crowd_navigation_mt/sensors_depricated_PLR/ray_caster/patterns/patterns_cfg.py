...
class GridPatternCfg(PatternBaseCfg):
    ...
    indexing_mode: Literal["xy", "ij"] = "xy"
    """Grid ordering. Defaults to "xy".

    If “xy” is selected, the first dimension corresponds to the cardinality of the second input and the second
    dimension corresponds to the cardinality of the first input.
...
class LidarPatternCfg(PatternBaseCfg):
    ...
    channels: int = 16
    """number of channels, vertical resolution of the lidar, default is 16"""

    vertical_fov_range: tuple[float, float] = (-15.0, 15.0)
    """Vertical field of view range in degrees, default is (-15.0, 15.0), i.e. 30 degrees. 0 is the horizontal lidar plane."""

    horizontal_fov_range: tuple[float, float] = (0.0, 360.0)
    """Horizontal field of view range in degrees, default is (0.0, 360.0), i.e. 360 degrees. 0 is the x axis. >0 is counter-clockwise."""

    horizontal_res: float = 0.2
    """Horizontal/ azimuth angular resolution in degrees, default is 0.2"""
