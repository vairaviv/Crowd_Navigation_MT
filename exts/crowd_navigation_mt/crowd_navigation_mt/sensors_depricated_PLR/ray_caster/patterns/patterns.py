...
def grid_pattern(cfg: patterns_cfg.GridPatternCfg, device: str) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    grid_x, grid_y = torch.meshgrid(x, y, indexing=cfg.indexing_mode)
...
def lidar_pattern(cfg: patterns_cfg.LidarPatternCfg, device: str) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    vertical_angles = torch.linspace(
        cfg.vertical_fov_range[0], cfg.vertical_fov_range[1], cfg.channels
    )

    # Horizontal angles: Adjust to avoid duplicating the endpoint in a full sweep
    horizontal_angle_range = cfg.horizontal_fov_range[1] - cfg.horizontal_fov_range[0]
    num_horizontal_angles = int(horizontal_angle_range / cfg.horizontal_res) + 1
    ...
    vertical_angles_rad = vertical_angles * (torch.pi / 180)
    horizontal_angles_rad = horizontal_angles * (torch.pi / 180)
