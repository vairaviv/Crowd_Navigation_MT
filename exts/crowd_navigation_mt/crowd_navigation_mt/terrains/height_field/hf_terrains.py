...
def discrete_obstacles_terrain(difficulty: float, cfg: hf_terrains_cfg.HfDiscreteObstaclesTerrainCfg) -> np.ndarray:
    ...
    # -- shape
    obs_width_range = np.arange(obs_width_min, obs_width_max + 1, 1)
    obs_length_range = np.arange(obs_width_min, obs_width_max + 1, 1)
    # -- position
    ...
    for _ in range(int(cfg.num_obstacles * difficulty)):
        # sample size
...
@height_field_to_mesh
def discrete_obstacles_terrain_wedge(
    difficulty: float, cfg: hf_terrains_cfg.HfDiscreteObstaclesWedgeTerrainCfg
) -> np.ndarray:
    """Generate a terrain with randomly generated obstacles as pillars with positive and negative heights.
    Additionally, the terrain has a wedge shaped obstacle to test memory and planning capabilities.
    ...
    # add wedge:
    wedge_width = np.random.uniform(*cfg.wedge_width_range)
    wedge_depth = np.random.uniform(*cfg.wedge_depth_range)
    wedge_width_pixels = int(wedge_width / cfg.horizontal_scale)
    wedge_depth_pixels = int(wedge_depth / cfg.horizontal_scale)
    thickness = int(cfg.wedge_thickness / cfg.horizontal_scale)
    center_x = width_pixels // 2

    for y_pos in range(wedge_depth_pixels):
        # x_pos = length_pixels - 1 - y_pos

        # Calculate left arm x positions
        left_x = -thickness // 2 + y_pos * (wedge_width_pixels // 2) // wedge_depth_pixels + center_x

        # Calculate right arm x positions
        right_x = thickness // 2 - y_pos * (wedge_width_pixels // 2) // wedge_depth_pixels + center_x

        # Draw thickness
        hf_raw[y_pos, left_x : left_x + thickness] = obs_height
        hf_raw[y_pos, right_x - thickness : right_x] = obs_height

    # random rotate
    rotation_choice = np.random.choice([0, 90, 180, 270])
    if rotation_choice == 90:
        hf_raw = np.fliplr(hf_raw.T)
    elif rotation_choice == 180:
        hf_raw = np.flipud(np.fliplr(hf_raw))
    elif rotation_choice == 270:
        hf_raw = np.flipud(hf_raw.T)
...
@height_field_to_mesh
def l_shaped_passage_terrain(difficulty: float, cfg: hf_terrains_cfg.HfDiscreteLShapedPassageTerrainCfg) -> np.ndarray:
    """Generate a terrain with an L-shaped passage that is randomly open or blocked.

    Args:
        difficulty: The difficulty of the terrain. This is a value between 0 and 1.
        cfg: The configuration for the terrain. Not used for this terrain.
    ...
    # -- L-shaped passage
    width = np.random.uniform(*cfg.l_width_range)
    length = np.random.uniform(*cfg.l_side_range)

    l_width = int(width / cfg.horizontal_scale)
    l_length = int(length / cfg.horizontal_scale)

    center = width_pixels // 2
    ...
    hf_raw[
        center - (l_width + wall_thickness) // 2 : center - (l_width - wall_thickness) // 2,
        center - (l_width + wall_thickness) // 2 : center - (l_width + wall_thickness) // 2 + l_length,
    ] = obs_height
    hf_raw[
        center + (l_width - wall_thickness) // 2 : center + (l_width + wall_thickness) // 2,
        center + (l_width - wall_thickness) // 2 : center - (l_width + wall_thickness) // 2 + l_length,
    ...
    # randomly block the passage
    hf_raw[
        center - (l_width + wall_thickness) // 2 : center + (l_width + wall_thickness) // 2,
        center
        - (l_width + wall_thickness) // 2
        + l_length : center
        - (l_width + wall_thickness) // 2
        + l_length
        + wall_thickness,
    ] = (
        obs_height if np.random.rand() < 0.5 else 0
    )

    # random mirror:
    if np.random.rand() < 0.5:
        hf_raw = np.flipud(hf_raw)
...
@height_field_to_mesh
def s_shaped_passage_terrain(difficulty: float, cfg: hf_terrains_cfg.HfDiscreteSShapedPassageTerrainCfg) -> np.ndarray:
    """Generate a terrain with an S-shaped passage that is randomly open or blocked.

    Args:
        difficulty: The difficulty of the terrain. This is a value between 0 and 1. Not used for this terrain.
        cfg: The configuration for the terrain.
    ...
    # -- L-shaped passage
    width = np.random.uniform(*cfg.s_width_range)
    side = np.random.uniform(*cfg.s_side_range)
    length = np.random.uniform(*cfg.s_length_range)

    s_width = int(width / cfg.horizontal_scale)
    s_side = int(side / cfg.horizontal_scale)
    l_length = int(length / cfg.horizontal_scale)
    ...
    # streight part:
    hf_raw[
        center - (s_width + wall_thickness) // 2 : center - (s_width - wall_thickness) // 2,
        center : center + l_length // 2 - s_width // 2,
    ] = obs_height
    hf_raw[
        center + (s_width - wall_thickness) // 2 : center + (s_width + wall_thickness) // 2,
        center : center + l_length // 2 + s_width // 2,
    ...
    # corner:
    hf_raw[
        center - s_width // 2 - s_side : center - s_width // 2 + wall_thickness // 2,
        center + (l_length - s_width - wall_thickness) // 2 : center + (l_length - s_width + wall_thickness) // 2,
    ] = obs_height
    hf_raw[
        center - s_width // 2 - s_side : center + s_width // 2 + wall_thickness // 2,
        center + (l_length + s_width - wall_thickness) // 2 : center + (l_length + s_width + wall_thickness) // 2,
    ...
    # other side
    hf_raw_copy = hf_raw.copy()
    flip = np.random.rand() < 0.5
    if flip:
        hf_raw_copy = np.fliplr(hf_raw_copy)
    else:  # filip and mirror
        hf_raw_copy = np.flipud(np.fliplr(hf_raw_copy))
        hf_raw_copy[:-1, :] = hf_raw_copy[1:, :]  # shift to align

    hf_raw[hf_raw_copy != 0] = obs_height

    # randomly block the passage
    hf_raw[
        center - s_width // 2 - s_side - wall_thickness // 2 : center - s_width // 2 - s_side + wall_thickness // 2,
        center + (l_length - s_width - wall_thickness) // 2 : center + (l_length + s_width + wall_thickness) // 2,
    ] = (
        obs_height if np.random.rand() < cfg.closes_prob else 0
    )
...
@height_field_to_mesh
def discrete_obstacles_cell_side_terrain(
    difficulty: float, cfg: hf_terrains_cfg.HfDiscreteObstaclesCellSideTerrainCfg
) -> np.ndarray:
    """Generate a terrain with randomly generated obstacles as pillars with positive and negative heights.
    ...
    difficulty_rescaled = difficulty / 2 + 0.5

    half_cell_wall_width = int(difficulty_rescaled * cfg.cell_wall_width / (cfg.horizontal_scale * 2))

    random_x_shift = np.random.uniform(*cfg.position_range) * difficulty
    random_y_shift = np.random.uniform(*cfg.position_range) * difficulty
    midx = int(width_pixels / 2 + random_x_shift / cfg.horizontal_scale)
    midy = int(length_pixels / 2 + random_y_shift / cfg.horizontal_scale)
    thickness = int(cfg.cell_wall_thickness / cfg.horizontal_scale)

    hf_raw[midx - half_cell_wall_width : midx + half_cell_wall_width, :thickness] = 5
    hf_raw[midx - half_cell_wall_width : midx + half_cell_wall_width, -thickness:] = 5
    hf_raw[:thickness, midy - half_cell_wall_width : midy + half_cell_wall_width] = 5
    hf_raw[-thickness:, midy - half_cell_wall_width : midy + half_cell_wall_width] = 5
...
def wall_terrain(difficulty: float, cfg: hf_terrains_cfg.HfWallTerrainCfg) -> np.ndarray:
    """Generate a terrain with a wall
    ...
    # generate the obstacles
    thickness = 8
    i = int(length_pixels * difficulty)
    start = max(0, i - thickness)

    if cfg.side == "left":
        hf_raw[:, start : start + thickness] = 5

    else:
        hf_raw[:, -thickness - start : -(start + 1)] = 5
