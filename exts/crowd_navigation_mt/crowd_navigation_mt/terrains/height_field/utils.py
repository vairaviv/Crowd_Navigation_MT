...
def height_field_to_mesh(func: Callable) -> Callable:
    ...
    def wrapper(difficulty: float, cfg: HfTerrainBaseCfg):
        ...
        # compute origin
        x1 = int((cfg.size[0] * 0.125 - 1) / cfg.horizontal_scale)
        x2 = int((cfg.size[0] * 0.125 + 1) / cfg.horizontal_scale)
        y1 = int((cfg.size[1] * 0.125 - 1) / cfg.horizontal_scale)
        y2 = int((cfg.size[1] * 0.125 + 1) / cfg.horizontal_scale)
...
def convert_height_field_to_mesh(
    ...
    # padd by zeros to avoid edge artifacts
    hf = np.pad(hf, pad_width=1, mode="constant", constant_values=0)
    # read height field
